<template>
  <div class="workbench-history">
    <div class="card-block card-transition" style="flex-direction: column; box-shadow: 0 0 3px #ddd; z-index: 1;">
      ...
      <el-alert title="每一个模板缓存缓存存储有限，超出将替换掉较老的缓存。" type="warning" effect="dark" :closable="false" show-icon style="margin-top: 16px;"/>
    </div>

    <div class="history-card" v-if="cardShow" :style="{'max-height': (maxHeight - (authSave ? 204 : 106)) + 'px'}">
...
import {zipPack} from '@/utils/zipBlive.js'
import {addHistory, delHistory, getHistory, listHistory} from "@/utils/history.js";
...
export default {
  ...
  methods: {
    ...
    updateView() {
      ...
      this.uuid = window.location.href.match(/([\w-]+)$/)[1];
      this.cardShow = true
    },
    // 通知工作台保存模板
    saveClick() {
      this.$store.commit('History/setIni')
    },
    listUse(uuid, value, time) {
      if (addHistory(uuid, value, time)) {
        ...
        this.$message({type: 'error', message: '使用失败。'});
      }
    },
    // 使用列表中的缓存
    ListUseClick(uuid, {value, time}) {
      if (this.uuid.indexOf('-') >= 0) {
        (async () => {
          await this.$alert('如果工作台中的内容未保存将丢失，确定继续吗？', '系统提示', {
            confirmButtonText: '确定',
            cancelButtonText: "取消",
            showCancelButton: true,
            type: 'warning',
          }).then(() => this.listUse(uuid, value, time)).catch(() => this.$message({message: `取消了，使用缓存。`, type: 'info'}));
        })()
      } else {
        this.listUse(uuid, value, time)
      }
    },
    // 导出缓存
    ListExportClick(uuid, time) {
      if (zipPack(uuid, time)) {
        this.$message({type: 'success', message: '模板导出成功。'})
      } else {
        this.$message({type: 'error', message: '模板导出失败，请联系管理员。'})
      }
    },
    // 删除某一个缓存
    listRemove(uuid, time) {
      let isAll = true;
      let history = getHistory(uuid, 'all')
      if (history.length <= 1) {
        (async () => {
          ...
          if (isAll) {
            if (delHistory(uuid, 0)) {
              this.$store.commit('History/setView')
              this.$message({type: 'success', message: '缓存删除成功。'})
            } else {
              this.$message({type: 'error', message: '删除缓存失败，请联系管理员。'})
            }
          } else {
            this.$message({message: `取消了，删除缓存。`, type: 'info',});
          }
        })()
      } else {
        if (delHistory(uuid, time)) {
          this.$message({type: 'success', message: '缓存删除成功。'})
        } else {
          this.$message({type: 'error', message: '删除缓存失败，请联系管理员。'})
        }
    ...
    ListRemoveClick(uuid, time) {
      // 判断是否只有一个缓存
      let isRemove = true;
      let history = getHistory(uuid)
      if (uuid === this.uuid && history.time === time) {
        (async () => {
          await this.$alert('检测到你正在删除自己正在编辑的模块，确定要删除吗。', '系统提示', {
            ...
            showCancelButton: true,
            type: 'warning',
          }).catch(() => isRemove = false);
          if (isRemove) {
            this.listRemove(uuid, time)
          } else {
            this.$message({message: `取消了，删除缓存。`, type: 'info',});
          }
        })()
